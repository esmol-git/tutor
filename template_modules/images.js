				...
				if (templateConfig.images.optimize.enable) {
					...
					for (const file of files) {
						...
						if (file.endsWith('.html') || file.endsWith('.js')) {
							...
							// Обработка изображений, указанных в srcset тегов SOURCE
							...
							// Обработка изображений, указанных в src тегов IMG
							...
							// Обработка изображений, указанных в href тегов A
							content = await replaceAsync(content, /<a\s[^>]*href=["']([^"']+\.(jpg|jpeg|avif|png|gif|webp))["'][^>]*>/gi, async (data, url) => {
								return await returnUrl(data, url)
							})
						} else if (file.endsWith('.css')) {
							// Обработка изображений, указанных в url CSS-файлов
...
// Конвертация и изменение размеров изображений
